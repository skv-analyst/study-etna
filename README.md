...
- Маршрут изучения — книга Артема Груздева **«Пособие по библиотеке временных рядов ETNA»**. Купить можно на Boosty автора за [1500руб](https://boosty.to/drains/posts/8b02f186-b785-403b-a371-7581b8929eab?share=post_link). Книга платная, поэтому PDF-файлы глав в этом репозитории **не публикуются**.
...
Зависимости `numba`/`llvmlite` для этой архитектуры не имеют прекомпилированных колёс и собираются из исходников — нужен компилятор LLVM:
...
Как и в случае с macOS Intel/Linux, готовые колёса `numba`/`llvmlite` под Windows есть на PyPI, поэтому дополнительный компилятор не требуется.
