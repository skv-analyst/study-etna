...
Идея: в старом докладе кор-разработчика ETNA прозвучал тезис, что в основе библиотеки лежат три базовые абстракции —
`TSDataset`, `Transforms`, `Model`. Сверил это с реальным кодом установленной версии 3.0.0
(`.venv/lib/python3.11/site-packages/etna`) — тезис подтверждается. Остальные 14 разделов
[API reference](https://docs.etna.ai/stable/api_reference.html) — это надстройки разного уровня над этими тремя
кирпичами, а не равноправный список.
...
Пронизывает всё, сама по себе не про временные ряды.
